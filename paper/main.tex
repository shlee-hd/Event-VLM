...
\begin{itemize}
    ...
    \item On UCF-Crime and XD-Violence pilot runs, Event-VLM delivers \textbf{$\sim$9$\times$} throughput gains while retaining near-baseline caption quality (98.8--99.4\% CIDEr retention). This supports real-time multi-camera deployment in the pilot regime.
\end{itemize}
...
Their visual stacks largely inherit ViT-family design advances~\cite{vit,deit,swin,pvt} and self-supervised pretraining paradigms~\cite{mae,dinov2}. For video understanding, Video-LLaMA~\cite{videollama} and VideoChat~\cite{videochat} extend image VLMs with temporal modeling. VILA~\cite{vila} further shows that video pre-training improves temporal reasoning. These models still employ heavy visual encoders with billion-parameter LLMs, and quadratic attention~\cite{attention} remains expensive for real-time deployment.
...
Traditional Video Anomaly Detection (VAD) relied on reconstruction errors~\cite{ucfcrime,vad_survey} or temporal features~\cite{rtfm,i3d}. Recent VLM-based approaches improved explainability. AnomalyGPT~\cite{anomalygpt} fine-tunes VLMs on defect datasets. Holmes-VAD~\cite{holmes_vad} adds multi-modal detection with chain-of-thought reasoning, and VADCLIP~\cite{vadclip} adapts CLIP for weakly supervised VAD. Weakly-supervised anomaly-learning lines~\cite{mist,mgfn,umil} provide strong localization priors, but they are not designed for caption-level explanation under strict latency budgets. Domain surveys in safety monitoring emphasize the gap between recognition-centric pipelines and operationally reliable explanation systems~\cite{industrial_safety_survey,construction_safety}. However, these methods mainly target offline single-stream processing.

\subsection{KV Cache Optimization and Sparse Attention}
The growing KV cache is a critical bottleneck for LLM inference. StreamingLLM~\cite{streamingllm} keeps a fixed-size window with ``attention sinks.'' H2O~\cite{h2o} evicts tokens by cumulative attention. SnapKV~\cite{snapkv} selects entries per layer, and PyramidKV~\cite{pyramidkv} allocates layer-wise budgets. However, these methods rely on heuristic importance signals and are not truly query-aware.
...
Table~\ref{tab:main_results_xd} shows that the same compute-quality pattern transfers to XD-Violence. Event-VLM-Core provides the highest practical throughput among explanation models. Event-VLM-Full recovers additional semantic quality with marginal latency overhead.
